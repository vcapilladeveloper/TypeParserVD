...
final class TypeParserVDTests: XCTestCase {
    ...
    func manageModelWithResult<T: Codable>() -> Result<T, Error> {
        let data = Data(usersJson.utf8)
    ...
    func test_givenValidJSONWithResult_completionWithUsers() {
        /// Given
        let result: Result<[User], Error> = manageModelWithResult()
        switch result {
    ...
    func test_getDataWithResult_givenMissingValueJSON_completionWithError() {
        ...
        let resultMissing: Result<[User], Error> = TypeParserVD.getData(from: dataMissing)
    ...
    func test_getDataWithResult_invalidTypeValueJSON_completionWithError() {
        ...
        let resultType: Result<[User], Error> = TypeParserVD.getData(from: dataInvalid)
        switch resultType {
        case .success(_):
            XCTAssertTrue(false)
